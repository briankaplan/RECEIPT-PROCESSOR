#!/usr/bin/env python3
"""
Direct Full Receipt Scan
Integrates with existing personalized email search system
"""
...
class DirectFullScanner:
    """
    Direct full scanner that uses your existing system
    """
    ...
    async def run_full_scan(self, start_date: str = "2024-07-01", 
        ...
        """
        Run full scan using existing personalized email search
        """
        ...
        while current_date <= end:
            ...
            # Scan this chunk
            chunk_results = await self._scan_chunk_with_existing_system(
    ...
    async def _scan_chunk_with_existing_system(self, start_date: str, end_date: str, 
                                             transactions: List[Dict]) -> Dict:
        """
        Scan a specific date chunk using existing personalized email search
        """
        chunk_results = {
            'emails_processed': 0,
            'receipts_found': 0,
        ...
        try:
            ...
            search_results = await self._run_personalized_search(days_back)
    ...
    async def _run_personalized_search(self, days_back: int) -> Dict:
        """
        Run personalized email search using existing system
        """
        try:
            # Import and use existing personalized search
            ...
            # Run the search
            results = await search_system.execute_personalized_search(days_back=days_back)
            
            return results
        
        except Exception as e:
            logger.error(f"❌ Error running personalized search: {e}")
            return {'emails': []}
    ...
    def _save_scan_results(self):
        ...
        """
        try:
            filename = f"direct_full_scan_results_{datetime.now().strftime('%Y%m%d_%H%M%S')}.json"
...
async def run_direct_full_scan(start_date="2024-07-01", end_date="2025-06-28"):
    """
    Run the direct full receipt scan
    """
    scanner = DirectFullScanner()
    return await scanner.run_full_scan(start_date, end_date)

if __name__ == "__main__":
    # Example usage
    async def main():
        results = await run_direct_full_scan()
        print(f"Direct scan complete! Found {results['statistics']['total_receipts']} receipts")
